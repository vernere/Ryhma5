...
const MainContent = () => {
    const {
        selectedNote, selectedNoteId, fetchNotes, activeUsers, updateNoteTitle, deleteNote,
    } = useNotesStore();
    ...
    return (<div className="flex-1 flex flex-col">
            <div className="bg-white border-b border-gray-200 p-2 flex items-center justify-between">
                {selectedNote ? (<div className="flex items-center space-x-4">
                        <div className="flex flex-col">
                            <div className="flex items-center gap-4">
                                <input
                                    data-cy="noteTitle"
                                    className="text-xl font-semibold text-gray-900 truncate max-w-2xl focus:outline-none"
                                    value={selectedNote.title || ""}
                                    onChange={(e) => updateNoteTitle(selectedNoteId, e.target.value)}
                                    placeholder="Title…"
                                />
                                {activeUsers.map((user) => (<span
                                        data-cy="userEmail"
                                        key={user.user_id}
            ...
            <div className="flex-1 p-6 overflow-y-auto bg-gray-50">
                {selectedNote ? (<div className="max-w-4xl mx-auto w-full">
                        <CollaborativeEditor Toolbar={Toolbar}/>
                    </div>) : (<div className="flex items-center justify-center h-full w-full pr-10">
